		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.109.0 oidcFederationProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
		...
		try {
			resp = await config.fetch(url, {
				...
				headers: {
					...
					"User-Agent": config.userAgent || `anthropic-sdk-typescript/0.109.0 userOAuthProvider`
				},
				body: JSON.stringify(body)
			});
		} catch (err) {
...
*/
var APPEND_HEADERS = new Set(["x-stainless-helper"]);
...
new TextEncoder();
//#endregion
//#region node_modules/@anthropic-ai/sdk/index.mjs
...
async function confineToRoot(root, p, opts) {
	...
	if (real !== realRoot && !real.startsWith(realRoot + path.sep)) throw new ToolError(`path ${JSON.stringify(p)} escapes workdir`);
	return real;
}
/**
...
async function extractSkillArchive(resp, dest) {
	...
	try {
		...
		for (const entry of await fs.readdir(srcRoot)) await fs.rename(path.join(srcRoot, entry), path.join(dest, entry));
	} finally {
